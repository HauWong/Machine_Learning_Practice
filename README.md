...
通过从上述![](http://latex.codecogs.com/gif.latex?\min_{w,b}\max_{\alpha}[])还是很难求得![](http://latex.codecogs.com/gif.latex?w)和![](http://latex.codecogs.com/gif.latex?b)。由于原问题为二次凸优化问题，且约束条件为线性条件，因此满足KKT条件（可以证明），可以转化为与原问题具有强对偶关系的对偶问题![](http://latex.codecogs.com/gif.latex?\max_{\alpha}\min_{w,b}[])。
...
![](http://latex.codecogs.com/gif.latex?\max_\alpha-\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^n\alpha_i\alpha_jy_iy_j\langle{x_i,x}\rangle+\sum_{i=1}^n\alpha_i,s.t.\sum_{i=1}^n\alpha_iy_i=0,\alpha_i\geq{0})，也就是
